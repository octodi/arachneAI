...
from src.gemini import consultantAI
app = Flask(__name__)
...
def web_vulnerability_scan(website_url):
    ...
    subprocess.check_output(['python', 'src/scanner.py', 'full', website_url])
    subprocess.check_output(['python', 'src/wappalyzer.py', '-u', website_url ,'-wf', report_file_name])
